...
const ImageCardSelect = ({ options = [], value, onChange, columns = 3 }) => {
  return (
      ...
      {options.map((option) => (
        <div key={option.value} className={`col-12 col-md-${12 / columns}`}>
          <StyledCard
            ...
            elevation={0}
          >
            <CheckIconWrapper className="position-absolute" selected={value === option.value}>
              <CheckCircleIcon fontSize="small" />
            </CheckIconWrapper>

            <CardContent className="text-center p-4 d-flex flex-column align-items-center h-100 justify-content-center">
              {option.icon && (
                <IconWrapper className="d-flex justify-content-center">{option.icon}</IconWrapper>
              )}
