...
const Breadcrumb = ({ config }) => {
    ...
    const renderSegment = (segment, index, isLast) => {
        ...
        if (isLast) {
            return <Typography key={index} color="text.primary">{content}</Typography>;
        }
        ...
        return <Typography key={index} color="inherit">{content}</Typography>;
    };
    ...
    return (
        ...
        >
            {segments.map((segment, index) => renderSegment(segment, index, index === segments.length - 1))}
        </Breadcrumbs>
    );
};
            ...
            PropTypes.shape({
                segments: PropTypes.arrayOf(
                    PropTypes.oneOfType([
                        ...
                        PropTypes.shape({ text: PropTypes.string, to: PropTypes.string }),
                    ])
                ).isRequired,
